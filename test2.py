...
indices_zpt = [(x,y) for x in range(n_preys) for y in range(n_time)]
indices_all_intercepted = [x for x in range(n_time)] 
...
def create_advanced_model():
    ...
    model.interception = pulp.LpVariable.dicts(
        "interception", indices=indices_zpt, lowBound=0, cat="Binary"
    )

    model.all_intercepted = pulp.LpVariable.dicts(
        "all_intercepted", indices=indices_all_intercepted, lowBound=0, cat="Binary"
    )

    # funzione obiettivo
    model += sum(model.var_path[x[0], x[1], x[2]] * costs[x] * (1 - model.all_intercepted[x[2]]) for x in indices)
    ...
    #interception
    var = []
    for index_prey, p in enumerate(path_preys):
        for t in times:
            for i1 in nodes:
                for i2 in nodes:
                    for j1 in nodes:
            ...
            model += sum(var) >= model.interception[index_prey, t]
    ...
    for t in times.difference({0}):
        for index_prey in range(len(path_preys)):
            model += model.interception[index_prey, t - 1] <= model.interception[index_prey, t]


    var_interception = []
    for t in times:
        for index_prey in range(len(path_preys)):
            var_interception.append(model.interception[index_prey, t])
        model += sum(var_interception) >= n_preys * model.all_intercepted[t]
...
#     prey2 = [i for i, j in path_preys[1].items() if j == 1] ############################# PREY 2
...
#     mem_usage = memory_usage(create_advanced_model)
